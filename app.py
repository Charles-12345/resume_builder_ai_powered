...
try:
    ...
    def render_footer() -> None:
        ...
        st.markdown(
            ...
            "Resume Builder AI™ - Built by DOKA CHARLES DANIEL<br>"
...
def payment_gate_ui(feature_label: str) -> bool:
    ...
    st.warning(f"🔒 {feature_label} is locked. Please unlock to download.")

    pay_link = get_secret("PAYMENT_LINK")
    if pay_link:
        st.link_button("💳 Pay to unlock", pay_link)
    ...
    if unlock_code_secret:
        ...
        st.info("ℹ️ Unlock code not configured. Contact administrator.")
...
def page_home() -> None:
    ...
    st.markdown(
        """
## Welcome to AI Resume & Cover Letter Builder! 🎉

This tool helps you create professional, ATS-optimized application materials.

### 🚀 What You Can Do
- 📄 Resume Builder (DOCX export)
- ✉️ Cover Letter Generator (DOCX export)
- ✅ ATS Checker (keyword matching + score)
- 📊 Dashboard (analytics)
...
def page_resume_builder() -> None:
    ...
    with col_left:
        st.markdown("### 1️⃣ Personal Details")
        ...
        st.divider()
        st.markdown("### 3️⃣ Professional Summary")
        ...
        if st.button("✨ Generate AI Summary", type="primary", use_container_width=True):
            if not full_name.strip() or not headline.strip():
                st.error("❌ Please provide at least your name and target title.")
            else:
    ...
    with col_right:
        st.markdown("### 4️⃣ Experience (Add Multiple)")
        ...
        with st.form("experience_form", clear_on_submit=True):
            ...
            add_ex = st.form_submit_button("➕ Add Experience", use_container_width=True)

        if add_ex:
            if not ex_title.strip() or not ex_company.strip() or not ex_bullets_raw.strip():
                st.error("❌ Provide role title, organization, and at least one bullet.")
            else:
            ...
            for i, item in enumerate(st.session_state.experience):
                with st.expander(f"{i+1}. {item.title} @ {item.company}", expanded=False):
                    ...
                    for b in item.bullets:
                        st.write(f"• {b}")
                    if st.button("🗑️ Remove", key=f"rm_ex_{i}"):
                        st.session_state.experience.pop(i)
                        st.rerun()
        else:
            st.info("ℹ️ No experience items yet.")

        st.divider()
        st.markdown("### 5️⃣ Education (Add Multiple)")
        ...
        with st.form("education_form", clear_on_submit=True):
            ...
            add_ed = st.form_submit_button("➕ Add Education", use_container_width=True)

        if add_ed:
            if not ed_degree.strip() or not ed_inst.strip():
                st.error("❌ Provide degree and institution.")
            else:
        ...
        if st.session_state.education:
            for i, ed in enumerate(st.session_state.education):
                with st.expander(f"{i+1}. {ed.degree}", expanded=False):
                    ...
                    if st.button("🗑️ Remove", key=f"rm_ed_{i}"):
                        st.session_state.education.pop(i)
                        st.rerun()
        else:
            st.info("ℹ️ No education items yet.")

    st.divider()
    st.markdown("### 6️⃣ Optional Sections")
    ...
    if st.button("📥 Generate Resume DOCX", type="primary", use_container_width=True):
        if not full_name.strip() or not email.strip():
            ...
            with st.spinner("📄 Generating your resume..."):
                ...
                st.success(f"✅ Resume generated: `{os.path.basename(path)}`")
    ...
    if st.session_state.generated_resume_path and os.path.exists(st.session_state.generated_resume_path):
        ...
        if payment_gate_ui("Resume DOCX download"):
            ...
            st.info("🔒 Complete payment/unlock to download your resume.")
    ...
    with col2:
        ...
        if st.button("📄 Export to DOCX", type="primary", use_container_width=True):
            if not cover_text.strip():
                st.warning("⚠️ No cover letter text to export.")
            else:
                ...
                st.success(f"✅ Cover letter saved: `{filename}`")
        ...
        if st.session_state.generated_cover_path and os.path.exists(st.session_state.generated_cover_path):
            ...
            if payment_gate_ui("Cover Letter DOCX download"):
                ...
                st.info("🔒 Complete payment/unlock to download your cover letter.")


def page_ats_checker() -> None:
    render_brand_sidebar()
    render_brand_header()
    st.subheader("✅ ATS Optimization Checker")
...
def page_legal() -> None:
    render_brand_sidebar()
    render_brand_header()
    st.subheader("⚖️ Legal / Trademark Notice")
